...
| `sanitize-terms.example` | `<kits home>/.sanitize-terms` | DP-5 | Copied and then **replaced with the user's real terms** at install. One plain term per line, matched case-insensitively. The search gate reads this file directly. |
| `sanitize-terms.cs.example` | `<kits home>/.sanitize-terms.cs` | DP-5 | Same, for names that collide with ordinary words: one `\bWord\b` pattern per line, matched case-sensitively. |
